...
def ve_start_inline_keyboard():
    kb = InlineKeyboardBuilder()
    kb.add(types.InlineKeyboardButton(text="Старт регистрации видеомонтажёра", callback_data="start_ve_registration"))
    return kb.as_markup()

def ve_confirmation_keyboard():
    kb = InlineKeyboardBuilder()
    kb.add(types.InlineKeyboardButton(text="Исправить", callback_data="ve_correct_data"))
    kb.add(types.InlineKeyboardButton(text="Все верно", callback_data="ve_confirm_data"))
    return kb.as_markup()
...
async def cmd_start_ve(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=ve_start_inline_keyboard(),
    )
...
async def start_ve_registration(callback: types.CallbackQuery, state: FSMContext):
    ...
    kb = types.ReplyKeyboardMarkup(
        keyboard=[[types.KeyboardButton(text="Заполнить форму видеомонтажёра", web_app=web_app)]],
        resize_keyboard=True,
        one_time_keyboard=True,
    )
    await callback.message.answer("Пожалуйста, заполните форму регистрации видеомонтажёра:", reply_markup=kb)
    await state.set_state(VideoEditorRegistrationStates.waiting_for_mini_app)
    await callback.answer()

async def handle_ve_webapp_data(message: types.Message, state: FSMContext):
    try:
    ...
    confirmation_text = (
        "<b>Проверьте введённые данные (Видеомонтажёр):</b>\n\n"
    ...
    await message.answer(confirmation_text, reply_markup=ve_confirmation_keyboard(), parse_mode="HTML")
